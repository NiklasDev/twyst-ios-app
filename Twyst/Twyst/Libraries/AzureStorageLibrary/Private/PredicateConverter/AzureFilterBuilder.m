	...
	switch([expr expressionType])
	{
		...
		default:
		{
			NSError* err = [NSError errorWithDomain:@"AzureFilterBuilder" 
											   code:-1
										   userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Unsupported expression type : %ld", (long)[expr expressionType]]
...
- (void) writeComparison:(NSPredicateOperatorType)predicateOperatorType left:(NSExpression*)left right:(NSExpression*)right
{
	switch(predicateOperatorType)
	{
		...
		default:
		{
			NSError* err = [NSError errorWithDomain:@"AzureFilterBuilder" 
											   code:-1
										   userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Unsupported operator type : %ld", (long)predicateOperatorType]
																				forKey:NSLocalizedDescriptionKey]];
			[self parserFailedWithError:err];
			break;
		}
