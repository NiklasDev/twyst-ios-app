...
- (void)getBlobDataWithContainer:(NSString*)containerName withBlobName:(NSString*)blobName withBlock:(void (^)(NSData *, NSError *))block {
    NSString* endpoint = [NSString stringWithFormat:@"/%@/%@", containerName, blobName];
    CloudURLRequest* request = [_credential authenticatedRequestWithEndpoint:endpoint forStorageType:@"blob", nil];
    ...
    [request fetchDataWithBlock:^(NSData* data, NSError* error)
     {
         ...
         {
             [_delegate storageClient:self didGetBlobData:data blob:nil];
         }
     }];
}
...
- (void)addBlobToContainerName:(NSString *)containerName blobName:(NSString *)blobName contentData:(NSData *)contentData contentType:(NSString*)contentType withBlock:(void (^)(NSError *))block {
    ...
    [request fetchNoResponseWithBlock:^(NSError* error)
     {
         ...
         {
             [_delegate storageClient:self didAddBlobToContainer:nil blobName:blobName];
         }
     }];
}
...
{
    queueName = [queueName lowercaseString];
    NSString* endpoint = [NSString stringWithFormat:@"/%@/messages?numofmessages=%ld", [queueName URLEncode], (long)fetchCount];
    if(peekOnly)
    {
...
#pragma mark - Clould URL Request Delegate
- (void)cloudURLRequest:(CloudURLRequest *)request totalBytesWritten:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite {
    if ([_delegate respondsToSelector:@selector(storageClient:totalBytesWritten:totalBytesExpectedToWrite:)]) {
        [_delegate storageClient:self totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite];
    }
}
